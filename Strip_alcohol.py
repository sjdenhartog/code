...
kLa_relations = [Empirical.Deckwer1, Empirical.Deckwer2, Empirical.Jackson_Shen, Empirical.Heijnen_VantRiet]
relation_names = ['Deckwer1', 'Deckwer2', 'Jackson and Shen', "Heijnen Van 't Riet"]
kLa_all = []
for name, relation in enumerate(kLa_relations):
    if relation == Empirical.Heijnen_VantRiet:
        kLa = relation(u_G, T_K)
        kLa_all.append(kLa)
    else:
        kLa = relation(u_G)
        kLa_all.append(kLa)
    ...
    ax1.plot(sol.t/3600, sol.y[0]*MW_eth/1000, label=relation_names[name]+r'$k_La$' + ' = ' + f'{round(kLa*3600, 0)}' + r'$h^{-1}$')
